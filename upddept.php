...
session_start();
	...
	$count=0;$resul='';
	$dname=isset($_POST['dname'])?trim($_POST['dname']):'';$warn1='';
	$hod=isset($_POST['hod'])?trim($_POST['hod']):'';$warn2='';
	pg_connect("host=localhost port=5432 dbname=placement user=dilshad password=a") or die("could not connect");
if (isset($_POST['update'])) 
{	
	if($hod=='')
	{
		$warn2="Enter HoD name ";
		$count=$count+1;
	}		
	if($count==0)
		$result=pg_query("update dept set hod='$hod' where dname='$dname'");	
	if($result)
	{	
		$resul="Update Successfull..";
		$dname='';
		$hod='';
	}
}
...
{
		$query=pg_query("SELECT dname FROM student where dname='$dname'");
		$row=pg_fetch_array($query);
		$query1=pg_query("SELECT dname FROM course where dname='$dname'");
		$row1=pg_fetch_array($query1);
		if($dname==$row[0]&&$dname==$row1[0])
		{
			$warn1="Department is associated with Student and Course";
			$count=1;
		}
		elseif($dname==$row[0])
		{
			$warn1="Department is associated with Student";
			$count=1;
		}
		elseif($dname==$row1[0])
		{
			$warn1="Department is associated with Course";
			$count=1;
		}			
	if($count==0)
		$result=pg_query("delete from dept where dname='$dname'");
	if($result)
	{	
...
<html xml:lang="en-gb" xmlns="http://www.w3.org/1999/xhtml" lang="en-gb"><head>
  <title>Update Department Detail</title>
...
<li class="parent active item69"><a href="" class="topdaddy"><span>Department</span></a>
<ul>
<li class="item70"><a href="subdept.php"><span>insert</span></a></li>
<li id="current" class="active item71"><a href="upddept.php"><span>Update</span></a></li></ul></li>
...
<a href="web.php" class="pathway">Home</a><img src="placement/arrow.png" alt=""> <a href="" class="pathway">Department</a><img src="placement/arrow.png" alt=""> Update</span>
<table class="contentpaneopen"><tbody><tr>
<td class="contentheading" width="100%">Update Department Details</td></tr>
<tr>
<td class="mod" width="100%"><?php echo $resul; ?></td></tr>
</tbody></table>
<form action="upddept.php" method="post">
	...
	<fieldset>
        ...
        <tr>
			<td class="text_b2">DepartmentName <span class="mandatory-star">*</span></td>
			...
			<tr>
          		<td class="text_b2">Head of Department <span class="mandatory-star">*</span></td>
          		<td><input type="text" name="hod" class="box4" value="<?php echo $hod?>"></td>
