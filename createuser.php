...
session_start();
	...
	$password=isset($_POST['password'])?trim($_POST['password']):'';$warn2='';
if (isset($_POST['submit'])) 
{
	pg_connect("host=localhost port=5432 dbname=placement user=dilshad password=a") or die("could not connect");
	if($name=='')
	{
		$warn1="Enter User name Unique";
		$count=1;
	}
	else
	{
		$query=pg_query("SELECT loginid FROM admin where loginid='$name'");
		$row=pg_fetch_array($query);
		if($name==$row[0])
		{
			$warn1="Username Already Exist";
			$count=1;
		}
	}	
	if($password=='')
	...
	{
		$warn2="Enter Password atleast 3 Character";
		$count=1;
	}		
	if($count==0)
		$result=pg_query("insert into admin values('$name','$password')");	
	if($result)
	{	
...
<li class="parent item56"><a href="" class="topdaddy"><span>Place Student</span></a>
<ul>
...
<?php
	...
	{
		echo '<li id="current" class="active item23"><a href="createuser.php"><span>Create User</span></a></li>';
...
<a href="web.php" class="pathway">Home</a><img src="placement/arrow.png" alt=""><a href="" class="pathway"><?php echo $_SESSION['name'];?></a><img src="placement/arrow.png" alt="">Create User</span>
<table class="contentpaneopen"><tbody><tr>
<td class="contentheading" width="100%">Create Users</td></tr>
<tr>
...
<form action="createuser.php" method="post">
	...
	<tr>
      		<td class="text_b2">Password<span class="mandatory-star">*</span></td>
