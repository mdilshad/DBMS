...
<html xml:lang="en-gb" xmlns="http://www.w3.org/1999/xhtml" lang="en-gb"><head>
  <title>Search Department Details</title>
<style>
 .mod {
color:red;
font-weight:bold;
...
<li id="current" class="active item71"><a href="serdept.php"><span>Search</span></a></li></ul></li>
...
<a href="query.php" class="pathway">Home</a><img src="placement/arrow.png" alt=""> <a href="" class="pathway"> Department </a><img src="placement/arrow.png" alt=""> Search </span>
														
<table class="contentpaneopen"><tbody><tr>
<td class="contentheading" width="100%">Department Search</td></tr>

</tbody></table>
<form action="serdept.php" method="post">
<table width="100%" border="0" cellspacing="2" cellpadding="2">
        <tbody>
        
        <tr>
          	<td><input type="text" name="dname" class="box4" value="<?php echo $dname; ?>"><input type="submit" name="dsearch" value="Search"></td>
        	</tr>
      	</tbody>
      	</table>
      	</form>
...
<table  style="width: 820px;" cellspacing="12" cellpading="10" border="1"><tbody>
	<?php
		...
		{
			$len=strlen($dname);
			$sql=pg_query("SELECT dname FROM dept");
			while($row=pg_fetch_row($sql))
				if(!strncasecmp($dname,$row[0],$len))
					$flag=1;
			if($flag==1)		
			{
				$sql=pg_query("SELECT * FROM dept");
				echo '<tr><th>Department Name</th><th>Head of Department</th></tr>';
				while($row=pg_fetch_row($sql))
				if(!strncasecmp($dname,$row[0],$len))
				{	
