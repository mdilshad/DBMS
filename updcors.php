...
session_start();
	...
	$offer=isset($_POST['offer'])?trim($_POST['offer']):'';$warn2='';
if (isset($_POST['update'])) 
{
	if($depname!='')
	{
			$result=pg_query("update course set dname='$depname' where cname='$cname'");
			if($result)
			{	
	...
	if($offer!=''&&$offer>0)
	{
			$sql=pg_query("select dname from course where cname='$cname' and dname='$depname'");
			$row=pg_fetch_row($sql);
			if($depname!=$row[0])
			{
				$warn2="Select Correct Department name";
			}
			else
			{
				$result=pg_query("update course set offers=(offers+$offer) where cname='$cname' and dname='$depname'");
				if($result)
				{	
					$resul="Update Successfull..";$offer='';
				}
			}
	}		
}
...
{
	$query=pg_query("SELECT * FROM student where cname='$cname'");
	$row=pg_fetch_array($query);
	if($cname==$row[0] and $dname==$row[1])
	{
		$warn1="Course is associated with Student";
		$count=1;
	}		
	if($count==0)
		$result=pg_query("delete from course where cname='$cname' and dname='$depname'");
	if($result)
	{	
...
<li id="current" class="active item68"><a href="updcors.php"><span>Update</span></a></li></ul></li>
...
<a href="web.php" class="pathway">Home</a><img src="placement/arrow.png" alt=""> <a href="" class="pathway">Course</a><img src="placement/arrow.png" alt="">Update</span>
<table class="contentpaneopen"><tbody><tr>
<td class="contentheading" width="100%">Update Course Details</td></tr>
<tr>
<td class="mod" width="100%"><?php echo $resul; ?></td></tr>
</tbody></table>
<form action="updcors.php" method="post">
	...
	<fieldset>
		...
		?>
         ...
         <tr>
          	<td class="text_b2">Number of New Offers from Company</td>
          	<td><input type="text" name="offer"  size="4" class="box4" value="<?php echo $offer?>"></td>
